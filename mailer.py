...
# ✅ CONFIGURATION
# -------------------------------------------------
FACTORY_NAME = "KOKKALATTY TEEA TRADERS"

def get_db_path():
    # Matches the logic in app.py to find KOKKALATTY.db
...
def generate_report_summary():
    ...
    summary += f"• Sales Transactions: {len(data['sales'])}\n"
    summary += f"• Total Revenue: Rs.{data['total_sales_value']:.2f}\n"
    summary += f"• Total Expenses Today: Rs.{data['total_expenses']:.2f}\n"
    summary += f"• Stock Inward Batches: {len(data['stock_inward'])}\n\n"

    # --- 2. SALES DETAILS ---
    summary += "1. TODAY'S SALES LOG:\n" + "-"*30 + "\n"
    if data['sales']:
        for sale in data['sales']:
            summary += f"• Bill #{sale['id']} | {sale['customer_name']} | Rs.{sale['total']} ({sale['payment_method']})\n"
    else:
    ...
    if data['stock_inward']:
        for item in data['stock_inward']:
            summary += f"• Batch: {item['batch_id']} | {item['name']} | Qty: {item['qty']} {item['unit']}\n"
    else:
    ...
    if data['expenses']:
        for exp in data['expenses']:
            summary += f"• {exp['description']} | Rs.{exp['amount']} [{exp['category']}]\n"
    else:
...
def send_report_email():
    ...
    receiver = "24bcs228@kgcas.com,mukunth2307@gmail.com"
